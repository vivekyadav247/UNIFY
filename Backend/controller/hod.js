...
async function getHodDashboard(req, res) {
  try {
    ...
    // Faculty count (TGs)
    const facultyCount = await TG.countDocuments({
      department: req.user.department,
      isActive: true,
    });
    ...
    const avgAttendance = attendanceRecords[0]?.avgAttendance || 0;

    // Pending tasks (leaves to approve)
    const pendingTasks = await Leave.countDocuments({
      status: "pending",
      department: req.user.department,
    });

    // Students at risk (low attendance)
    const studentsAtRisk = await ClassAttendance.countDocuments({
      department: req.user.department,
...
// Get all TGs for HOD's department
async function getTGList(req, res) {
  try {
    const tgList = await TG.find({
      department: req.user.department,
    }).select("-password");
    ...
    console.error("Error fetching TG list:", error);
    res.status(500).json({
      success: false,
      error: error.message,
    });
...
// Create new TG
async function createTG(req, res) {
  try {
    const { name, email, phone, department, qualifications, experience, specialization } =
      req.body;

    // Check if TG already exists
    const existingTG = await TG.findOne({ email });
    if (existingTG) {
      return res.status(400).json({
        success: false,
        error: "Teacher Guardian with this email already exists",
      });
    }
    ...
    res.json({
      success: true,
      message: "Teacher Guardian created successfully",
      tg: newTG,
    });
  } catch (error) {
...
// Get TG by ID
async function getTGById(req, res) {
  try {
    const tg = await TG.findById(req.params.tgId).select("-password");

    if (!tg) {
      return res.status(404).json({
        success: false,
        error: "Teacher Guardian not found",
      });
    }
    ...
    console.error("Error fetching TG:", error);
    res.status(500).json({
      success: false,
      error: error.message,
    });
...
// Update TG
async function updateTG(req, res) {
  try {
    const { name, phone, qualifications, experience, specialization, isActive } =
      req.body;
    ...
    res.json({
      success: true,
      message: "Teacher Guardian updated successfully",
      tg,
    });
  } catch (error) {
    console.error("Error updating TG:", error);
    res.status(500).json({
      success: false,
      error: error.message,
    });
...
async function deleteTG(req, res) {
  try {
    const tg = await TG.findByIdAndDelete(req.params.tgId);
    ...
    res.json({
      success: true,
      message: "Teacher Guardian deleted successfully",
    });
  } catch (error) {
    console.error("Error deleting TG:", error);
    res.status(500).json({
      success: false,
      error: error.message,
    });
