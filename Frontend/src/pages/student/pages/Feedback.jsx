...
import React, { useState } from "react";
import { useOutletContext } from "react-router-dom";
import { FiMessageCircle, FiCalendar, FiStar } from "react-icons/fi";
import FeedbackCategoryFilter from "../components/FeedbackCards/FeedbackCategoryFilter";
import FeedbackDetailCard from "../components/FeedbackCards/FeedbackDetailCard";
import SendFeedbackForm from "../components/FeedbackCards/SendFeedbackForm";
...
export default function Feedback() {
  ...
  const feedbackData = [
    {
      ...
      title: "Excellent Performance in Data Structures",
      message:
        "You have shown exceptional understanding of data structures concepts. Your implementation of AVL trees was particularly impressive. Keep up this excellent work!",
      rating: 5,
    },
    {
      id: 2,
      ...
      title: "Time Management Focus",
      message:
        "While your technical knowledge is solid, I suggest you work on time management for your projects. Setting milestones and deadlines for yourself will help you deliver better quality work.",
      rating: 4,
    },
    {
      id: 3,
      ...
      title: "Great Classroom Participation",
      message:
        "Excellent participation in class discussions. Your questions demonstrate deep thinking about the subject matter. Continue asking insightful questions!",
      rating: 5,
    },
  ];
  ...
  const filteredFeedback =
    activeCategory === "all" ? feedbackData : feedbackData.filter((f) => f.category === activeCategory);
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-start">
        <div>
          <h1 className={`text-4xl font-bold ${darkMode ? "text-white" : "text-gray-900"}`}>Feedback 💬</h1>
          <p className={`mt-2 text-lg ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
            Receive constructive feedback from your mentors
          </p>
        </div>
      ...
      {showForm && <SendFeedbackForm darkMode={darkMode} onClose={() => setShowForm(false)} />}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div
          className={`p-6 rounded-2xl border transition-all duration-300 ${
            darkMode
              ? "bg-gradient-to-br from-green-900/30 to-green-800/20 border-green-700/40 hover:border-green-600/60"
              : "bg-gradient-to-br from_green-50 to_green-100/50 border_green-200/60 hover:border_green-300"
          } hover:shadow-lg`}
        >
          <div className="flex items-center justify-between mb-4">
            <div className={`p-3 rounded-lg ${darkMode ? "bg-green-700/40" : "bg-green-200"}`}>
